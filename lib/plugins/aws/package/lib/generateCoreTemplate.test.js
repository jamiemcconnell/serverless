...
describe('#generateCoreTemplate()', () => {
  ...
  it('should use a custom cloudformation description', () => {
    const cfDescription = 'Custom Description';
    awsPlugin.serverless.service.provider.description = cfDescription;
    awsPlugin.serverless.service.provider.deploymentBucket = "com.serverless.deploys";
    ...
    return awsPlugin.generateCoreTemplate()
      .then(() => {
        expect(awsPlugin.serverless.service.provider.compiledCloudFormationTemplate.Description)
          .to.equal(cfDescription);
      });
  });
